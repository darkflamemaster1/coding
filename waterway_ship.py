...
#第n艘船减速进航道的速度,a表示输入的i，b表示输入的j，p为位置矩阵，Lx为航道在x方向的位置，Ly为在y方向的位置，an_x为x方向的加速度，an_y为y方向的加速度，ret为需要输出的结果
#def vn(p,a,b,Lx,Ly,an_x,an_y,ret):
#        dx=Lx-p[a]
#        dy=Ly-p[b]
#        vnx=int((2*an_x*dx)**0.5)
#        vny=int((2*an_y*dy)**0.5)
#        if dx>((vnx**2)/(2*an_x)) and dx=p[a+1]-p[a]:
#                vnx=min(vnx+an_x,vmax_x)
#        elif dx<((vnx**2)/(2*an_x)):
#                vnx=max(vnx-an_x,vnx)
#        if dy>((vny**2)/(2*an_y)) and dy=p[b+1]-p[b]:
#                vny=min(vny+an_y,vmax_y)
#        elif dy<((vny**2)/(2×an_y)):
#                vny=max(vny-an_y,vny)
#        if ret=='vnx':
#                return vnx
#        elif ret=='vny':
#                return vny
#        elif ret=='vn'
#                return int((vnx**2+vny**2)**0.5)
#        elif ret=='theta':
#                return int(np.arctan(vny/vnx))
